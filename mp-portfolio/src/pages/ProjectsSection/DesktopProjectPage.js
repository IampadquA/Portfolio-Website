...
export const DesktopProjectPage = ({ isDesktop = false }) => {
    ...
    const x = useTransform(scrollYProgress, [0.25,0.32, 0.39, 0.46, 0.6, 0.7, 0.75, 0.8], [initialTransformValue, "-100%","-100%","-100%", "-200%", "-200%", "-200%", "-300%"]);
    const y = useTransform(scrollYProgress, [0.4,0.46, 0.5, 0.75, 0.8, 0.85, 0.95], [initialTransformValue, "-25%", "-25%", "-30%", "-30%", "-30%", "-50%"]);
    const openerHeight = useTransform(scrollYProgress, [0.5, 0.7], [initialTransformValue, "100%"]);

    const opacity = useTransform(scrollYProgress, [0.5, 0.55], [0, 1]);
    const translateY = useTransform(scrollYProgress, [0.5, 0.55], [50, 0]);

    const eduPageTextY = useTransform(scrollYProgress, [0.42, 0.46], ["-0%","-50%" ]);
    ...
    const text = [
        ...
        "Technologies Used",
        "Frontend: React | Styling: Tailwind CSS |   Animations: Framer Motion | \n Backend: Firebase Functions, Node.js | \n Database: Firebase \n",
        "Key Features",
            ...
            <motion.div className="sticky top-0 w-screen h-screen flex items-start justify-center overflow-hidden">
                <motion.div className='flex w-screen' style={{ x: x || "0%", y: y || "0%" }}>
                    ...
                    <div className='w-screen h-[200vh] flex flex-shrink-0 '>
                        <Educhain>
                            <ScrollingText contents={texts} y={eduPageTextY} />
                        </Educhain>
                    </div>
                    <div className='w-screen h-[200vh] flex flex-shrink-0 '>
                        <motion.div
